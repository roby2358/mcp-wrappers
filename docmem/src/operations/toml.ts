...
export async function importToml(toml: string): Promise<ToolResponse> {
  ...
  const nodes: { id: string; parentId: string | null; context: { type: string; name: string; value: string }; content: string }[] = [];
  ...
  // Multi-pass: keep inserting nodes whose parents are already inserted
  ...
  for (const node of sorted) {
    ...
    const hash = computeHash(
      node.parentId, node.context.type, node.context.name, node.context.value,
      node.content, currentOrder,
    );
...
export async function exportToml(nodeId: string): Promise<ToolResponse> {
  ...
  const sections: Record<string, TomlNode> = {};
